...
def find_best_4_filaments(target_labs, library_fils, progress_cb=None):
    ...
    try:
        import numpy as np
        t_arr = np.array(target_labs, dtype=np.float32)          # (n_t, 3)
        f_arr = np.array([f["lab"] for f in library_fils],
                         dtype=np.float32)                        # (n_f, 3)
        # dist[i, j] = ΔE between target i and filament j
        diff = t_arr[:, None, :] - f_arr[None, :, :]             # (n_t, n_f, 3)
        dist = np.sqrt((diff ** 2).sum(axis=2))                   # (n_t, n_f)
    ...
    # ── Phase 1: Greedy — pick 4 filaments one by one ────────────────────────
    chosen = []
    ...
    # ── Phase 2: Local search — swap until no improvement ────────────────────
    improved = True
    iteration = 0
